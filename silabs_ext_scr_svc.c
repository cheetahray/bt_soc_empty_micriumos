...
static void rc_opr_broadcast_scene(void) __attribute__((unused));
...
static uint8_t rdev_using_scene_ctx(void * ptr) __attribute__((unused));
static uint8_t rdev_using_scene_ctx(void * ptr)
{
	...
	int popcnt=0;
	for(int idx=0; (unsigned)idx<ARRAY_SIZE(chk_mark); idx++) {
...
{
	uint32_t chk_mark[(256/32)]={0};
	for(int scene_idx=0; (unsigned)scene_idx<ARRAY_SIZE(RC_scene); scene_idx++) {
...
{
	uint32_t chk_mark[(256/32)]={0};
	for(int scene_idx=0; (unsigned)scene_idx<ARRAY_SIZE(RT_scene); scene_idx++) {
...
static void get_curt_msg_in(void) __attribute__((unused));
